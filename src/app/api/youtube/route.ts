...
// Cache simple en mémoire (sera réinitialisé à chaque redémarrage du serveur)
type CacheData = {
  videoData: {
    id: string;
    title: string;
  ...
  liveData: { isLive: boolean; url: string | boolean };
  channelData: { subscriberCount: string | null };
} | {
  videoData: null;
  liveData: { isLive: boolean; url: string | boolean };
};
...
export async function GET() {
  ...
  console.log("start youtube API route");
  ...
  if (!channelId) {
    return NextResponse.json(
      { error: "Paramètre channelId manquant" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    // --- Préparer liveData ---
    let liveData: { isLive: boolean; url: string | boolean } = { isLive: false, url: false };
    ...
    try {
      ...
      console.warn("Erreur lors de la récupération des abonnés :", e instanceof Error ? e.message : String(e));
    }
